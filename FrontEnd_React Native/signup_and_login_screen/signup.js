...
// Verify account with name email and phone number.
const VerifyAccount = () => {
  ...
  const handleNext = () => {
    if (validateForm()) {
    // setLoading(true); 
    axios.post('http://192.168.0.105:4000/api/v2/user/register', {
      phoneNumber,
      name,
      email,
    })
    ...
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      ...
      >
        <View style={styles.verifyAccountView}>
          ...
          <TouchableOpacity onPress={() => {navigation.navigate("Login")}} style={styles.alredyAccount}><Text>Alredy an Account?.</Text></TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
